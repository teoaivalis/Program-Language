...
real_checking(N,K,New_List):-
 ...
 read_function(0, _Stream, Answers, Final_Answers):-
   Final_Answers = Answers.

 read_function(M, Steam, CurrentAnswers, Answers):-
     read_line(Steam, [N, K]),
     real_checking(N, K, ResultR),
     append(CurrentAnswers, [ResultR], NewCurrentAnswers),
     NewM is M - 1,
     read_function(NewM, Steam, NewCurrentAnswers, Answers).
 ...
 powers2(File, Answers) :-
     ...
     read_line(Stream, M),
     [H | _] = M,
     read_function(H, Stream, [], Answers),!.
